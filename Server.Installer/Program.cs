...
namespace Server.Installer
{
    ...
    public class Program
    {
        ...
        public static async Task Main(string[] args)
        {
            ...
            Console.WriteLine(AppConstants.nexRemoteFreeAscii);
            ...
            {
                ConsoleHelper.WriteError("nex-RemoteFree server można zainstalować tylko w systemie Linux lub Windows.");
                return;
            }
            ...
            ConsoleHelper.WriteLine("Usługi Informatyczne nex-IT Jakub Potoczny.  Ten instalator poprowadzi Cię " +
                "poprzez wdrożenie serwera nex-RemoteFree na tym komputerze.");
            ...
            {
                cliParams.InstallDirectory = ConsoleHelper.ReadLine("Do którego katalogu należy rozpakować pliki serwera (np. /var/www/nex-RemoteFree/)?").Trim();
            }
        ...
        {
            ConsoleHelper.WriteLine("nex-RemoteFree Server Installer", 0, ConsoleColor.Cyan);
            ConsoleHelper.WriteLine("Builds a customized nex-RemoteFree server using GitHub actions " +
            ...
            ConsoleHelper.WriteLine("\t--github-username, -u    Your GitHub username, where the forked nex-RemoteFree repo exists.", 1);
            ...
            ConsoleHelper.WriteLine("\t--server-url, -s    The public URL where your nex-RemoteFree server will be accessed (e.g. https://remote.nex-it.pl).", 1);

            ConsoleHelper.WriteLine("\t--install-directory, -i    The directory path where the server files will be installed (e.g. /var/www/nex-RemoteFree/).", 1);
            ...
            ConsoleHelper.WriteLine("\t--web-server, -w    Number.  The web server that will be used as a reverse proxy to forward " +
                "requests to the nex-RemoteFree server.  Select the appropriate option for your operating system and web server.  " +
            ...
            ConsoleHelper.WriteLine("sudo ./nex-RemoteFree_Server_Installer -b false -u nexitpl -p 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 -s https://remote.nex-it.pl -i /var/www/nex-RemoteFree/ -r latest -c true -w 0", 1);

            ConsoleHelper.WriteLine("Example (use pre-built package):");
            ConsoleHelper.WriteLine("sudo ./nex-RemoteFree_Server_Installer -b true -s https://remote.nex-it.pl -i /var/www/nex-RemoteFree/ -w 0");
        }
    }
}
